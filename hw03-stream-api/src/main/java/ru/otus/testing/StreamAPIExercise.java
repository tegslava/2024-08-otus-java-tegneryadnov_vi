...
class StreamAPIExercise {
    ...
    static List<Task> getTasksListWithStatus(Stream<Task> taskStream, TaskStatus taskStatus) {
        return taskStream.filter(t -> t.getTaskStatus().equals(taskStatus)).toList();
    }
    ...
    static long getTaskCountWithStatus(Stream<Task> taskStream, TaskStatus taskStatus) {
        return taskStream.filter(t -> t.getTaskStatus().equals(taskStatus)).count();
    }
    ...
    static boolean conditionIsMet(Stream<Task> taskStream, long existsId, long notExistsId) {
        var taskSetIds = taskStream.map(Task::getId).collect(Collectors.toSet());
        return taskSetIds.contains(existsId) && !taskSetIds.contains(notExistsId);
    }
